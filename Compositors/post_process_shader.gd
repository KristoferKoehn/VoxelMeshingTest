...
float get_linear_depth(vec2 uv) {
	float linear = texture(depth_texture, uv).r;
	vec3 ndc = vec3(uv * 2.0 - 1.0, linear);
...
color = vec4(color.rgb * depth_border, 1.0);
"
...
func _check_shader() -> bool:
	...
	#new_shader_code = template_shader.replace("#COMPUTE_CODE", outline)
...
func _render_callback(p_effect_callback_type, p_render_data):
	...
	if rd and _check_shader():
		...
		if render_scene_buffers:
			...
			push_constant.push_back(0.0)
			for vec in inv_proj_mat_array:
				push_constant.push_back(vec.x)
				push_constant.push_back(vec.y)
				push_constant.push_back(vec.z)
				push_constant.push_back(vec.w)
			push_constant.push_back(timer % 1800);
			push_constant.push_back(0.0)
			push_constant.push_back(0.0)
			push_constant.push_back(0.0)
			...
			for view in range(view_count):
				...
				var depth_sampler = rd.sampler_create(RDSamplerState.new())
				...
				var depthUniform : RDUniform = RDUniform.new()
				depthUniform.uniform_type = RenderingDevice.UNIFORM_TYPE_SAMPLER_WITH_TEXTURE
				depthUniform.binding = 1
				depthUniform.add_id(depth_sampler)
				depthUniform.add_id(render_scene_buffers.get_depth_layer(0))
