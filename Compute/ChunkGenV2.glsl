...
vec2 displacement(vec2 p) {
    float n = fbm2D(p, 10.0);
    float angle = n * 6.2831;
    return vec2(cos(angle), sin(angle)) * 0.2;
}


int biomeIDFromVoronoi(vec3 pos, float seed) {
    vec2 cell = floor(pos.xz);
    vec2 frac = fract(pos.xz);
    ...
    for (int x = -1; x <= 1; ++x) {
		for (int z = -1; z <= 1; ++z) {
			vec2 neighbor = vec2(x, z);
			vec2 latticeCell = cell + neighbor;

			// Get random point inside this cell
